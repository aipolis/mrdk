/** 九宫格：API 无 grid9 时从 indicators / metrics 组装 */
...
function fromIndicators(indicators) {
  ...
  const map = {}
  indicators.forEach((item) => { map[item.key] = item })
...
function fromMetrics(metrics, prevMetrics) {
  ...
  const trend = (cur, p, inverse = false) => {
...
export function normalizeGrid9(data) {
  if (!data) return []
  ...
  if (fromInd.length || fromMet.length) return mergeToNine(fromInd, fromMet)
  return []
}
